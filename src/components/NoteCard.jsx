...
import { getNoteBodyPlain } from '../utils/noteContentModel.js';
import ComedyRatingTrigger from './ComedyRatingTrigger.jsx';
import styles from '../styles/NoteCard.module.css';

/** @param {{ note: { id: string, title: string, bodyHtml?: string, bodyMarkdown?: string, contentType?: string, comedyRating?: number | null, labels?: string[] } }} props */
export default function NoteCard({ note }) {
  const location = useLocation();
  const fromState = { from: `${location.pathname}${location.search}` };
  const preview = getNoteBodyPlain(note).replace(/\s+/g, ' ').trim();
